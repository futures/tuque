...
class RepositoryFactory {
  ...
  protected static function getVersionFedora3(RepositoryConfig $config) {
    $response = file_get_contents($config->getURL() . "/describe?xml=true");
    if ($response) {
      $element = new SimpleXMLElement($response);
      $element->registerXPathNamespace('default', 'http://www.fedora.info/definitions/1/0/access/');
      $result = $element->xpath('//default:repositoryVersion');
      if (isset($result[0])) {
